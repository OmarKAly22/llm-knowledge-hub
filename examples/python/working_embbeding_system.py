...
class EmbeddingEngine:
    ...
    def _generate_embedding(self, text: str) -> np.ndarray:
        ...
        keywords = ['rag', 'retrieval', 'vector', 'embedding', 'llm', 
                   'ai', 'database', 'search', 'query', 'document']
        ...
        features.append(len([w for w in text.split() if w[0].isupper()]) / max(len(text.split()), 1))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.use_cache:
            ...
            if stats['total_embedded'] > 0:
                stats['cache_hit_rate'] = stats['cache_hits'] / (stats['cache_hits'] + stats['total_embedded'])
            else:
...
# Sample documents
documents = [
    ("doc1", "RAG systems combine retrieval with generation for accurate AI responses."),
    ...
    ("doc5", "Retrieval-augmented generation improves LLM accuracy with external knowledge."),
]

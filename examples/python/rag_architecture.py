...
class Document:
    ...
    def __str__(self):
        content_preview = self.content[:100] if isinstance(self.content, str) else f"<{self.content_type} data>"
...
class ModernRAGSystem:
    ...
    def query(self, query: str, k: int = 3) -> Dict[str, Any]:
        ...
        for doc, score in results:
            response['results'].append({
                'doc_id': doc.doc_id,
                'content': doc.content[:200] if isinstance(doc.content, str) else "<binary>",
                'score': float(score),
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.metrics['queries_processed'] > 0:
            self.metrics['cache_hit_rate'] = self.metrics['cache_hits'] / self.metrics['queries_processed']
        else:

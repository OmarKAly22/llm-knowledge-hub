...
class MockVectorDB:
    ...
    async def search(self, query_embedding, k=10, filters=None):
        ...
        for doc, emb in self.storage:
            ...
            results.append(SearchResult(doc_id=doc.doc_id, content=doc.content, score=score, metadata=doc.metadata))
...
class WeaviateVectorDB(VectorDatabase):
    ...
    def __init__(self,
        ...
        if auth_key:
            self.client = weaviate.Client(
                url=url,
                auth_client_secret=AuthApiKey(api_key=auth_key)
            )
        else:
    ...
    async def search(self,
        ...
        query = (
            self.client.query
            .get(self.class_name, ["content", "metadata", "doc_id"])
...
async def main():
    ...
    docs = [
        ...
        Document("doc2", "Vector databases store embeddings.", {"type": "db"}, datetime.now()),
        Document("doc3", "Weaviate supports hybrid search.", {"type": "weaviate"}, datetime.now())
    ]
